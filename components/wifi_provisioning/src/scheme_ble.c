...
#if defined(CONFIG_MESH_LITE_WIFI_PROV_BLE_BONDING)
    ble_config->ble_bonding = 1;
#endif

#if defined(CONFIG_MESH_LITE_WIFI_PROV_BLE_SEC_CONN) || defined(CONFIG_BT_BLUEDROID_ENABLED)
    ble_config->ble_sm_sc = 1;
#endif

#if defined(CONFIG_MESH_LITE_WIFI_PROV_BLE_FORCE_ENCRYPTION)
    ble_config->ble_link_encryption = 1;
#endif

#if defined(CONFIG_MESH_LITE_WIFI_PROV_BLE_NOTIFY)
...
static esp_err_t set_config_service(void *config, const char *service_name, const char *service_key)
{
    ...
    (ESP_IDF_VERSION > ESP_IDF_VERSION_VAL(5, 2, 2) && ESP_IDF_VERSION < ESP_IDF_VERSION_VAL(5, 3, 0)) || \
...
#ifndef CONFIG_MESH_LITE_WIFI_PROV_KEEP_BLE_ON_AFTER_PROV
#ifdef CONFIG_BT_CONTROLLER_ENABLED
