...
esp_err_t app_espnow_trigger_distribution_network(void)
{
    esp_err_t ret = ESP_FAIL;
    cJSON *object = cJSON_CreateObject();
    cJSON *payload = cJSON_CreateObject();
    if (object) {
        if (payload) {
            cJSON_AddNumberToObject(payload, ESPNOW_DISTRIBUTION_NETWORK, 1);
            cJSON_AddItemToObject(object, ESPNOW_DEVICE_NAME, payload);
            char *rsp_string = cJSON_PrintUnformatted(object);
            if (rsp_string) {
